...
async function getConventionalRecommendedBump() {
  ...
  logNotice("conventional-commits", `${bumpResult.reason}: ${bump}`);
  return bump;
}
...
async function recommendedVersion(
  currentVersion: string,
  prereleaseId: string,
  ...
  if (forcePrerelease) {
    ...
    logNotice("prerelease-logic", prereleaseType);
...
async function main() {
  ...
  const forceReleaseCandidate = args.includes("--release-candidate");
  let forcePrerelease = args.includes("--prerelease");

  if (forceReleaseCandidate) {
    forcePrerelease = true;
    defaults.prereleaseId = "rc";
  }
  ...
  if (!pathExists([".git/"])) {
    logError("No '.git' directory found.");
  }
  ...
  if (isAutomated) {
    newVersion = await recommendedVersion(
      currentVersion,
      defaults.prereleaseId,
      forcePrerelease
    );
    logNotice("version-upgrade", `${currentVersion} => ${newVersion}`);
  } else {
    const suggestedVersion = await recommendedVersion(
      currentVersion,
      defaults.prereleaseId,
      forcePrerelease
    );
